		...
		if(_authType=='Bearer'){
			...
			if(_ignore.includes(trimSlash(req.route.path))==false){
				...
				if(_checkRole==true) {
					...
					// let _checkDBFilter = await Function.checkDBFilter(req,req.account.role_id);
					// Object.keys(_checkDBFilter).forEach(key => {
					// 	if(/event_id:(\d*)/.test(_checkDBFilter[key])){
					// 		req.headers['x-event-id']=_checkDBFilter[key].match(/event_id:(\d*)/i)[1];
					// 	}
					// 	req.query={...req.query,[key]:req.query?.[key] ? req.query[key] + ','+_checkDBFilter[key]:_checkDBFilter[key]}
					// })
				}
			}
