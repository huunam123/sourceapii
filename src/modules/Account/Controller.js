    ...
    try {
      ...
      if (_check == false && req.account) {
        ...
        if (
          ...
          _account.permission_apis = await this.getPermissionApis(_account.id,_account.role_id);
    ...
    return new Promise(async(resolve, reject) => {
      ...
      if(role_id != ''){
          ...
          if (result) {
              ...
              let _permission = result?.permissions ? JSON.parse(result?.permissions) : {};   
        ...
        if(_type==0){
          await _modules.data.forEach(async(v) => {
            if((_id !== process.env.ADMIN_ID && (v?.module in _permissions) && _permissions[v?.module].read == true && v?.status && v.status && v?.module && v?.name) || (_id == process.env.ADMIN_ID && v?.status && v.status && v?.module && v?.name)){
              ...
              if (n0 !== '' && !(n0 in r)) {
                  r[n0] = r[n0] ?? {'lv' : 0,'title' : n[0], 'items' : {}, 'sort_order':0, 'is_function':v.is_function};
              }
              if ((n0 in r) && n1 !== '' && !(n1 in r[n0].items)) {
                  r[n0].items[n1] = r[n0].items[n1] ?? ((n2 !== '') ? {'lv' : 1,'title' : n[1], 'items' : {},'is_function': v.is_function} : {'lv' : 1,'title' : n[1], 'module' : v.module, 'sort_order' : v.sort_order, 'is_function': v.is_function}) ;
              }
