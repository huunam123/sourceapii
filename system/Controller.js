...
module.exports = class Controller {
    constructor(tableName) {
        ...
        if(tableName=='customer_datas')
            this.db = new Model('customers');
        this.moment = moment;
        this.s3 = s3;
    }
    ...
    async uploadFile(fileName, base64Data, type, oldFile = '') {
        return new Promise(async (resolve, reject) => {
            try {
                const _buf = Buffer.from(base64Data.replace(/^data:(.*?)\/\w+;base64,/, ''), 'base64');
                const _data = {
                    ACL: 'public-read',
                    Key: fileName,
                    Body: _buf,
                ...
                s3.upload(_data, async (err, data) => {
                    if (err) {
                        ...
                        if (oldFile != '' && oldFile.indexOf('events/default')==-1) await s3.deleteObject({ Key: oldFile }).promise();
                        resolve(true);
                    }
                });
            } catch (e) {
